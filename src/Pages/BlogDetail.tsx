  ...
  return (
    <div className='flex flex-col gap-[104px] items-center'>
      <div className='flex flex-col items-end gap-[16px]'>
        <div className='flex flex-col gap-[64px] items-center w-full pb-[64px] border-b border-solid border-[#6E706E]'>
          <div className='flex flex-col gap-[16px] items-center'>
            <p className='text-[30px] font-medium text-center w-full leading-[42px] text-black'>{blogDetail?.title}</p>
            <div className='flex flex-row gap-[8px] items-center'>
              <div className='flex flex-row gap-[4px] px-[16px] py-[6px] rounded-[30px] border-solid border-[1px] border-[#6E706E] bg-white'>
                <IconTag />
                <div className='text-[18px] text-[#6E706E]'>{blogDetail?.category}</div>
              </div>
              <div className='text-[18px] text-[#6E706E] px-[16px] py-[6px] rounded-[30px] border-solid border-[1px] border-[#6E706E] bg-white'>{blogDetail?.date}</div>
            </div>
          </div>
          <div>
            <div className='text-[20px] text-left w-full leading-[28px]'>{blogDetail?.content[0]}</div>
            <img src={blogDetail?.imageBlog} alt="" />
            <div className='text-[20px] text-left w-full leading-[28px]'>{blogDetail?.content[1]}</div>
          </div>
        </div>
      ...
      <div className='flex flex-col gap-[32px] items-center'>
        <div className='text-[24px] font-bold text-center border-solid border-b border-[#000000] pb-[16px]'>Comment</div>
        <div className='flex flex-col gap-[16px] items-center'>
          {/* comment đây anh */}
